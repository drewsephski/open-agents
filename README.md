...
This repository is a fork of [Open Agents](https://github.com/vercel-labs/open-agents) from Vercel Labs. Launchstack is built on that open-source architecture — the web UI, agent runtime, sandbox orchestration, and GitHub integration — and is not the upstream Vercel Labs hosted product.

The repo is meant to be forked and adapted, not treated as a black box.

## What it is

Launchstack is a three-layer system:
...
You do not need a separate GitHub OAuth app. Launchstack uses the GitHub App's OAuth credentials as a Better Auth social provider, plus the App's installation tokens for repo access.
